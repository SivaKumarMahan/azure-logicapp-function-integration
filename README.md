...
# Get storage key
az storage account keys list -g aimsplus -n stoargefunctionlogicapp --query "[0].value" -o tsv
...
az monitor app-insights component show -g aimsplus -a mypythonfunctionappinsights --query instrumentationKey -o tsv
...
az sql server list --resource-group aimsplus --query "[].{name:name, fqdn:fullyQualifiedDomainName}"
[
  {
...
az sql db list --resource-group aimsplus --server sqlfunctionserverdemo --query "[].name"
[
  "master",
  "functiondb"
]

az sql server show --name sqlfunctionserverdemo --resource-group aimsplus --query "administratorLogin"
"sqladmin"
